...
<div class="publications">
  <div class="publication-browser">
    <p class="publication-browser__intro">
      Explore our work by research topic. Topics overlap, so a paper may appear in more than one collection.
    </p>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const publicationItems = Array.from(
    publicationContainer.querySelectorAll('.bibliography > li > .item')
  );
  const pageSize = 30;
  let activeTopic = 'all';
  ...
  function updateResults() {
    ...
    publicationItems.forEach(function(item) {
      item.classList.toggle('is-visible', visibleSet.has(item));
    });
